...
// router.get('/', async(req, res) => {  
// 	let ArchivoJson = await axios.get(`https://api.spoonacular.com/recipes/complexSearch?apiKey=${API_PASSWORD}&number=100&addRecipeInformation=true`)
// 	ArchivoJson=ArchivoJson.data.results
// 	for (let i=0; i<ArchivoJson.length; i++) {
// 		for (let j=0; j<ArchivoJson[i].diets.length; j++) {
// 			let [diet, created] = await Diet.findOrCreate({ 
// 				where: { name:  ArchivoJson[i].diets[j]
